...
Use the bundled Python 3.11+ runtime at `scripts/crg.pyz`; resolve paths relative to this skill, not the current workspace. Prefer `python3.13` when available. No pip dependencies are needed.

## Route the request

- Test installation: run `python3.13 <skill>/scripts/self_test.py`. This uses synthetic events in an automatically removed temporary workspace; it does not consume model usage or install hooks. Report the actual result and distinguish synthetic validation from live Desktop behavior.
...
Installing this skill does not install global hooks or provide continuous token telemetry. Existing project MODE_B hooks can warn, save a handoff and block an at-risk submission; a new Desktop task is created manually. Do not claim automatic UI switching. Separate owned App Server sessions are not the active Desktop connection.
